...
#define PUMP1 PORTDbits.RD3
#define PUMP2 PORTDbits.RD2
...
void main()
{
	...
	while(1){
		...
		if(INTCONbits.TMR0IF==1&&PORTAbits.RA5==1)
		{
			_ccp3Set(ZAxis.need,ZAxis.last);//inc is left dec is right
			_ccp2Set(LArm.need,LArm.last);//inc is extend
		...
		if(PIR1bits.ADIF==1)
		{
			...
			else if (readSel==3)//AN2//water level sensor 1
			{
				holdd=ADRESL;
				numInt=0;
				numInt=ADRESH;
			...
			else if (readSel==4)//AN2//water level sensor 1
			{
				holdd=ADRESL;
				numInt=0;
				numInt=ADRESH;

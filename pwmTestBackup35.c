...
T0[6]=(tray0.timeLeft/10000%10)+48;
T0[7]=(tray0.timeLeft/1000%10)+48;
T0[8]=(tray0.timeLeft/100%10)+48;
T0[9]=(tray0.timeLeft/10%10)+48;
T0[10]=(tray0.timeLeft%10)+48;
T1[6]=(tray1.timeLeft/10000%10)+48;
T1[7]=(tray1.timeLeft/1000%10)+48;
T1[8]=(tray1.timeLeft/100%10)+48;
T1[9]=(tray1.timeLeft/10%10)+48;
T1[10]=(tray1.timeLeft%10)+48;
LCD_SETPOS(1,0);
LCD_PRINT(T0);
LCD_SETPOS(2,0);
LCD_PRINT(T1);
...
void main()
{
	...
	while(1){
		...
		if(INTCONbits.TMR0IF==1&&doorLSValue==0)
		{
			_ccp3Set(ZAxis.need,ZAxis.last);//inc is left dec is right
			_ccp2Set(LArm.need,LArm.last);//inc is extend
		...
		if(INTCONbits.TMR0IF==1)
		{
			...
			INTCONbits.TMR0IF=0;
				...
				if(counter==19)
				{
					...
					{
						secondCounter=0;
						if(tray0.timeLeft>0)tray0.timeLeft--;
						if(tray1.timeLeft>0)tray1.timeLeft--;
						timeChangeFlag=1;
					}
				}
		...
		if(PIR1bits.ADIF==1)
		{
			...
			//	LArm.need=herp2;//x1
				...
				if(firstRun==1)
				{
					if(numCalc>=22&&numCalc<=24)
					{
					tray0.location=2;
					tray1.location=1;
					}
					else if (numCalc>=27&&numCalc<=29)
					{
					tray0.location=1;
					tray1.location=0;
					}
					else if (numCalc>=24&&numCalc<=26)
					{
					tray0.location=2;
					tray1.location=0;
					}
		...
		else if ((movingHomeStop==0&&tray0.location==1)||(movingHomeStop==0&&tray1.location==1))
		{
		...
		if(movingHomeStop==0&&timeChangeFlag==1)
		{
		timeChangeFlag=0;
		_PRINT_TIME();
		}

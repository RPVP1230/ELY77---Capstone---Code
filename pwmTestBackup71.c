	...
	while(1){
		...
		if(PIR1bits.ADIF==1)
		{
			...
			//	LArm.need=herp2;//x1
				...
				if(firstRun||doorLSValue!=0||tilted)
					{
					LCD_SETPOS(1,0);
		...
		if(promptFlag==1&&doorLSValue==0)
		{
			if(BUTTON1==0&&butID==0)
			{
				butID=1;
			}
			else if(BUTTON2==0&&butID==0)
			{
				butID=2;
			}
			else if(BUTTON3==0&&butID==0)
			{
				butID=3;
			}
			else if(BUTTON4==0&&butID==0)
			{
				butID=4;
			}
			butID=_PROMPT_USER(butID);
			if(butID==5)
				{
				butID=0;
				promptFlag=0;

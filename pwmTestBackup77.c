...
char _PROMPT_USER(char id,char initTrigger,char firstRun)
{
static char index=0;
char prompt0[]="Tray:  Change?      ";
char prompt1[]="^:empty \//:new seeds";
char prompt2[]="<: no change        ";
char prompt3[]=">: harvested        ";
if(id!=0)index++;
if(shelf[index].occ==0)index++;
if(shelf[index].occ==0)index++;
if(shelf[index].occ==0)index++;
	if(index<3)
	{
	...
	else if(firstRun==1&&initTrigger==1)
		{
		seedPrompt();
		}
	}
...
void main()
{
	...
	while(1){
		...
		if(INTCONbits.TMR0IF==1&&doorLSValue==0&&firstRun==0&&promptFlag==0)
		{
		...
		if(INTCONbits.TMR0IF==1)
		{
			...
			INTCONbits.TMR0IF=0;
				...
				if(counter==19)
				{
					...
					{
						secondCounter=0;
						if(shelf[0].timeLeft>0&&shelf[0].occ==1)shelf[0].timeLeft--;
						if(shelf[1].timeLeft>0&&shelf[1].occ==1)shelf[1].timeLeft--;
						if(shelf[2].timeLeft>0&&shelf[2].occ==1)shelf[2].timeLeft--;
						timeChangeFlag=1;
					}
				}
		...
		else if (movingHomeStop==0&&shelf[0].timeLeft<shelf[2].timeLeft&&firstRun==0)	
		{
			opID=1;
			movingHomeStop=1;
			operation=0;
		...
		else if (movingHomeStop==0&&shelf[1].occ==1&&firstRun==0)
		{
			if(shelf[0].occ==1)
			{
			opID=1;
		...
		if(promptFlag==1&&doorLSValue==0||firstRun)
		{
			...
			butID=_PROMPT_USER(butID,displayTrigger,firstRun);
			displayTrigger=0;
			if(butID==5)
				{
				butID=0;

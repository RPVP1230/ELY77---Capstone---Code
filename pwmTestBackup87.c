	...
	while(1){
		...
		if(PIR1bits.ADIF==1)
		{
			...
			else if (readSel==5)//temp reading
			{
				...
				numInt+=holdd;
				numCalc=numInt;
				numCalc=numCalc/1024.0;//290 is regular temp//283 will be too hot value
				if(numCalc<=283.0)
				{
				FAN1=1;
				FAN2=1;
		...
		if(changedFlag==1&&movingHomeStop==0&&firstRun==0&&promptFlag==0)//if the user emptied, right will always need more time than left, just rinse and swap while rinsing
		{
			opID=2;
			movingHomeStop=0;
			operation=0;
		...
		else if (movingHomeStop==0&&shelf[0].timeLeft<shelf[2].timeLeft&&firstRun==0&&promptFlag==0)	
		{
			opID=1;
			movingHomeStop=1;
			operation=0;
		...
		else if (movingHomeStop==0&&shelf[1].occ==1&&firstRun==0&&promptFlag==0)
		{
			if(shelf[0].occ==1)
			{
			opID=1;
